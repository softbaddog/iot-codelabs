...
int uart_data_read(char *buf, int maxrlen, int mode, int timeout)
{
    ...
    {
        //block mode
        retlen = los_dev_uart_read(LOS_STM32L452_UART2, buf, maxrlen, timeout);
    }
    else
    {
        //none block mode
        retlen = los_dev_uart_read(LOS_STM32L452_UART2, buf, maxrlen, 0);
    }
...
int uart_data_write(char *buf, int writelen, int mode)
{
    ...
    los_dev_uart_write(LOS_STM32L452_UART2, buf, writelen, 500);
    return 0;
}
